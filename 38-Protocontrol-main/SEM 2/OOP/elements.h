...
class Component {
public:
    virtual void draw(Adafruit_ILI9341 &tft);
    virtual void handleTouch(int x, int y, Adafruit_ILI9341 &tft);
    virtual void printVals();
    static std::vector<Component*> profile;
...
public:
    PushButton(int x, int y, unsigned long color, unsigned long textColor, const char text[]);
    PushButton();
    PushButton(char inputStr[]);
    void printVals();
...
PushButton::PushButton(int x, int y, unsigned long color, unsigned long textColor, const char text[]) {
    this->x = x;
    this->y = y;
    this->color = color;
...
PushButton::PushButton(char inputStr[]){
  char *ptr;
  char st[strlen(inputStr) + 1];
  strcpy(st, inputStr);
  this->compType = 'P';
  char *token = strtok_r(st, ",", &ptr);
  if (token != NULL){
    ...
    if (token != NULL){
      ...
      if (token != NULL){
        ...
        token = strtok_r(NULL, ",", &ptr);
        if (token != NULL){
          this->color = strtoul(token, NULL, 16);
          //Serial.println(this->color);
          token = strtok_r(NULL, ",", &ptr);
          if (token != NULL){
            this->textColor = strtoul(token, NULL, 16);
            //Serial.println(this->textColor);
            token = strtok_r(NULL, ",", &ptr);
            if(token != NULL){
              strncpy(this->text, token, sizeof(this->text));
              //Serial.print("TEXT: ");
...
void PushButton::draw(Adafruit_ILI9341 &tft) {
...
void componentFactory::build(String s){
  switch(s[0]){
    case 'P':
      PushButton(strtok((char*)s.c_str(),"|"));
    break;
  }
}
...
void Toggle::draw(Adafruit_ILI9341 &tft){
  if(this->state){
...
void increment::draw(Adafruit_ILI9341 &tft){
...
class Slider: public Component{
  public: 
    Slider(int x, bool vertical, int min, int max);
    void draw(Adafruit_ILI9341 &tft);
    void handleTouch(int x, int y, Adafruit_ILI9341 &tft);
    int value;
    int min;
 ...
 Slider::Slider(int x,bool vertical, int min, int max){
...
void Slider::draw(Adafruit_ILI9341 &tft){
  if(isVertical){
    ...
    tft.fillCircle(FRAME_X+(FRAME_W/2),FRAME_Y+(FRAME_H/2),25,ILI9341_BLACK);
  }else{
    tft.fillRect(FRAME_X,FRAME_Y,FRAME_W,FRAME_H,ILI9341_BLUE);
    tft.drawRect(FRAME_X,FRAME_Y,FRAME_W,FRAME_H,ILI9341_BLACK);
...
void Slider::handleTouch(int x, int y, Adafruit_ILI9341 &tft){
