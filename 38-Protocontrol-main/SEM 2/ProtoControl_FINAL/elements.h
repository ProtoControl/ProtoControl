...
void PushButton::handleTouch(int px, int py, Adafruit_ILI9341 &tft) {
    ...
    if(px < (PushButtonW + this->x) && px > (this->x) ){
      if((py > (sy - (this->y + PushButtonH))) && py < (sy - this->y)){
        ...
        Serial.println((String)this->location+(String)this->pressed);
        Serial1.println((String)this->location+(String)this->pressed);

        // sending[0] = this->location;
        // sending[1] = (char)!this->pressed;
        // //Serial.write(sending,2);
        // Serial1.write(sending,2);
      } 
    }
}
  ...
  if(this->state){
    if((x > REDBUTTON_X) && (x < (REDBUTTON_X + REDBUTTON_W))) {
        if ((y > REDBUTTON_Y) && (y <= (REDBUTTON_Y + REDBUTTON_H))) {
          ...
          //Serial.write((char)this->location);
          
          // sending[0] = this->location;
          // sending[1] = (char)!this->state;
          // Serial.write(sending,2);
          // Serial1.write(sending,2);
...
class Console{
  public: 
    ...
    void displayMessageln(const char text[], Adafruit_ILI9341 &tft);
    String message;
  private:
    int FRAME_X;
    int FRAME_Y;
...
void Console::displayMessageln(const char text[], Adafruit_ILI9341 &tft){
  //tft.fillRect(FRAME_X,FRAME_Y,FRAME_W,FRAME_H,ILI9341_BLACK);
  //tft.drawRect(FRAME_X,FRAME_Y,FRAME_W,FRAME_H,ILI9341_WHITE);
  tft.setTextColor(ILI9341_WHITE);
  tft.setTextSize(2);
  tft.setCursor(this->FRAME_X+24,this->FRAME_Y+24);
  tft.println(text);
}
